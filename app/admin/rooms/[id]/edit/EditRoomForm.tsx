...
export function EditRoomForm({ room }: { room: { id: string; name: string } }) {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '600px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        ...
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--text-primary)' }}>Modifier la salle</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', marginTop: '0.25rem' }}>{room.name}</p>
        </div>
      </div>
      ...
      >
        <div>
          ...
          <input
            name="name" required defaultValue={room.name}
            style={{
